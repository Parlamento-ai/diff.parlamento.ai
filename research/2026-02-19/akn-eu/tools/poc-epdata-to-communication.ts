...
const procYear = process.argv[2] || "2024";
const DIRECT_PROC_ID = process.argv[3]; // Optional: skip search, use this procedure ID directly
...
const OUTPUT_DIR = "samples/communication";
...
async function main() {
  ...
  const fileRef = procLabel
    ...
    .replace(/\(/g, "-")
    .replace(/\)/g, "");
  const outputPath = `${OUTPUT_DIR}/eu-communication-${fileRef}.xml`;
