...
// Defaults
METADATA.author = METADATA.author || '#ep';
...
METADATA.base = METADATA.base || '#com-proposal';
METADATA.result = METADATA.result || '#ep-position';
		...
		if (art.isNew) {
			...
			changes.push(
				`      <akndiff:articleChange article="${art.articleId}" type="insert"${afterAttr}>` +
					`\n        <akndiff:new><p>${escXml(newText)}</p></akndiff:new>` +
					`\n      </akndiff:articleChange>`
			);
		} else if (oldText && newText) {
			changes.push(
				`      <akndiff:articleChange article="${art.articleId}" type="substitute">` +
					`\n        <akndiff:old><p>${escXml(oldText)}</p></akndiff:old>` +
					`\n        <akndiff:new><p>${escXml(newText)}</p></akndiff:new>` +
					`\n      </akndiff:articleChange>`
			);
		}
...
<akomaNtoso xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0"
  ...
  <amendment name="${m.name}">
    <meta>
      ...
      <references>
        <TLCOrganization href="https://www.europarl.europa.eu" showAs="${m.authorShowAs}"/>
      </references>
    </meta>
    <preface>
    ...
    <akndiff:changeSet base="${m.base}" result="${m.result}">
      <akndiff:vote date="${m.voteDate}" result="${m.voteResult}" source="${m.author}">
