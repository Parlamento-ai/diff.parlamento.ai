...
const TOC_MODE = cliArgs.includes("--toc");
const positionalArgs = cliArgs.filter(a => !a.startsWith("--"));
const CELEX = positionalArgs[0] || "32018R1645";
const OUTPUT_DIR = positionalArgs[1] || "samples/act";
...
async function main() {
  ...
  for (const docUrl of docUrls) {
    ...
    if (ct.includes("zip")) {
      ...
      // TOC mode: extract the TOC file instead of the body
      if (TOC_MODE) {
...
function saveTocResult(xml: string) {
  mkdirSync(OUTPUT_DIR, { recursive: true });
  const outPath = `${OUTPUT_DIR}/${CELEX}-toc-formex.xml`;
