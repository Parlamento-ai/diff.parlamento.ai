...
function main() {
  ...
  // Build lookup maps keyed by the article num text
  const originalMap = new Map<string, Article>();
  for (const art of originalArticles) {
    const key = articleKey(art.num);
    originalMap.set(key, art);
  }

  const consolidatedMap = new Map<string, Article>();
  for (const art of consolidatedArticles) {
    const key = articleKey(art.num);
    consolidatedMap.set(key, art);
  }

  // Compare
  const changes: Change[] = [];

  // Collect all unique keys preserving order (original first, then any new ones)
  ...
  for (const key of allKeys) {
    ...
    if (orig && cons) {
      ...
      if (orig.textContent !== cons.textContent || orig.heading !== cons.heading) {
        changes.push({
          type: "substitution",
          key,
          articleNum: cons.num,
      ...
      // Only in consolidated — insertion
      changes.push({
        type: "insertion",
        key,
        articleNum: cons.num,
