...
const citOutputArg = args.find(a => a.startsWith("--output="))?.split("=")[1];
...
const OUTPUT_PATH = citOutputArg || `samples/citation/eu-citation-plenary-${year}.xml`;
...
function fetchJson(url: string): Promise<any> {
  return new Promise((resolve, reject) => {
    const doRequest = (targetUrl: string, redirects = 0) => {
      ...
      https.get(targetUrl, { headers: { Accept: "application/ld+json", "User-Agent": "Mozilla/5.0 poc-epdata-to-akn" }, checkServerIdentity: epCheckServerIdentity }, (res) => {
...
async function main() {
  ...
  let agendaItems: any[] = [];
  const foreseenUrl = `https://data.europarl.europa.eu/api/v2/meetings/${meetingId}/foreseen-activities?format=application%2Fld%2Bjson`;
  const decisionsUrl = `https://data.europarl.europa.eu/api/v2/meetings/${meetingId}/decisions?format=application%2Fld%2Bjson&offset=0&limit=50`;
