...
const billArgs = process.argv.slice(2);
const billOutputArg = billArgs.find(a => a.startsWith("--output="))?.split("=")[1];
const OUTPUT_DIR = billOutputArg ? dirname(billOutputArg) : "samples/bill";
...
function getTextContent(node: XmlNode): string {
  ...
  // Skip footnote reference anchors — their marker numbers bleed into article text
  if (node.tag === "a" && node.attrs?.["class"]?.includes("footnoteRef")) return "";
...
/** Match any Titrearticle variant: Titrearticle, Titrearticleb, Titrearticle0, Titrearticlef, Titrearticlefa */
function isArticleTitleClass(node: XmlNode): boolean {
  const classes = (node.attrs?.["class"] ?? "").split(/\s+/);
  return classes.some(c => /^Titrearticle/.test(c));
}
...
function extractBill(root: XmlNode, celex: string): ExtractedBill {
  ...
  if (legislativeStart >= 0) {
    ...
    let seenPoint0 = false; // tracks if current paragraph has Point0-level points
    ...
    while (cursor < allElements.length) {
      ...
      // Handles all EU XHTML patterns:
      //   DMA:      two ChapterTitle elements ("Chapter I" + "Subject matter")
      //   Data Act: combined ChapterTitle with <br/> ("CHAPTER I\nGENERAL PROVISIONS")
      //   DSA:      combined ChapterTitle with dash ("Chapter I – General provisions")
      //   NIS2:     Normal class with chapter text ("CHAPTER II" + Normal title)
      //   CSRD:     no chapters (amending directive) — falls through to implicit chapter
      {
        const isChapterEl = hasClass(el, "ChapterTitle");
        const chapterNumMatch = text.match(/^(CHAPTER\s+[IVXLCDM]+)/i);

        if (isChapterEl || chapterNumMatch) {
          flushChapter();
          let chapterNum = "";
          let chapterTitle = "";

          if (chapterNumMatch) {
            chapterNum = chapterNumMatch[1].trim();
            // Extract title from remaining text (combined with <br/> or dash)
            const rest = text.substring(chapterNumMatch[0].length);
            chapterTitle = rest.replace(/^[\s\n\r\u2013\u2014\u2015–—-]+/s, "").trim();
          } else {
            // ChapterTitle class without "CHAPTER X" pattern (orphaned title element)
            chapterTitle = text;
          }

          currentChapter = { num: chapterNum, title: chapterTitle, sections: [], articles: [] };
          cursor++;

          // If no title found in same element, look ahead
          if (!chapterTitle && cursor < allElements.length) {
            const nextEl = allElements[cursor];
            const nextText = getTextContent(nextEl).trim();
            const isValidTitle = !nextText.match(/^(CHAPTER|Article|TITLE|SECTION)\s+/i);
            if (isValidTitle && (hasClass(nextEl, "ChapterTitle") || (nextEl.attrs?.["class"] ?? "").split(/\s+/).includes("Normal"))) {
      ...
      // Handles all EU XHTML patterns:
      //   Standard:   Titrearticle class (DSA, NIS2, CSRD, AI Act)
      //   Data Act:   Titrearticleb, Titrearticle0, Titrearticlef, Titrearticlefa
      //   Combined:   "Article 1\nSubject matter" in single element (DMA, Data Act, CSRD)
      //   Split:      "Article 1" + next element with title (DSA, NIS2)
      //   NIS2:       title in following Normal element
      if (isArticleTitleClass(el)) {
        const artMatch = text.match(/^(Article\s+\d+[a-z]*)/i);
        if (artMatch) {
          flushArticle();
          const artNum = artMatch[1].trim();
          // Extract heading from remaining text (combined with <br/>)
          let heading = text.substring(artMatch[0].length).replace(/^[\s\n\r]+/s, "").trim();

          currentArticle = { num: artNum, heading, paragraphs: [] };
          cursor++;

          // If no heading found in same element, look ahead
          if (!heading && cursor < allElements.length) {
            const nextEl = allElements[cursor];
            const nextText = getTextContent(nextEl).trim();
            const notAnother = !nextText.match(/^Article\s+\d+/i);
            if (notAnother) {
              if (isArticleTitleClass(nextEl)) {
                currentArticle.heading = nextText;
                cursor++;
              } else if ((nextEl.attrs?.["class"] ?? "").split(/\s+/).includes("Normal") &&
                         nextText.length < 200 &&
                         !nextText.match(/^(CHAPTER|TITLE|SECTION)\s+[IVXLCDM]+/i) &&
                         !nextText.match(/^\d+\./)) {
                // NIS2 pattern: article title in Normal element
                currentArticle.heading = nextText;
                cursor++;
              }
            }
      ...
      if (cls.includes("Point0") || cls.includes("Point1") || cls.includes("Point2")) {
        ...
        if (cls.includes("Point2")) {
          // Point2 is always a sub-point
        ...
        } else if (cls.includes("Point1") && !cls.includes("Point0")) {
          // Point1: sub-point if preceded by Point0, otherwise top-level
          if (!currentParagraph) {
            currentParagraph = { num: "", content: "", points: [] };
          }
          if (seenPoint0 && currentParagraph.points.length > 0) {
            const lastPoint = currentParagraph.points[currentParagraph.points.length - 1];
            lastPoint.subpoints.push({ num, text: pointText });
          } else {
...
async function main() {
  const celex = billArgs.find(a => !a.startsWith("--")) || DEFAULT_CELEX;
  const url = `${CELLAR_BASE}/${celex}`;
  const outputPath = billOutputArg || `${OUTPUT_DIR}/${celex}-bill-akn.xml`;
  ...
  const rawPath = billOutputArg
    ? join(dirname(billOutputArg), `${celex}-raw.xhtml`)
    : `${OUTPUT_DIR}/${celex}-raw.xhtml`;
