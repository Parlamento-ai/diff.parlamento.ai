...
// We override checkServerIdentity only for this specific host.
...
function epCheckServerIdentity(hostname: string, cert: tls.PeerCertificate): Error | undefined {
  if (hostname === EP_HOST) {
    ...
    if (hasWildcard) return undefined; // accept
  }
...
const BASE_DIR = resolve(TOOLS_DIR, "data");
...
async function discoverProcedure(procedureCode: string): Promise<DiscoveredConfig> {
  ...
  return {
    ...
    comDate: epData.comDate ?? sparql.allCelex.find(r => r.celex === billCelex)?.date ?? null,
  };
}
...
async function runPipeline(config: DiscoveredConfig) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 4: EP amendments
  // ═══════════════════════════════════════════════════════════════════════════
  {
    const id = "ep-amendments", num = 4, name = "EP amendments";
    const t0 = Date.now();
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    const id = "viewer-xmls", num = 5, name = "Viewer XMLs";
    const t0 = Date.now();
    ...
    if (blockMsg) {
      ...
      if (ok) {
        ...
        const withContent = existing.filter((f) => {
          ...
          return f.startsWith("01-") || f.startsWith("03-")
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    const id = "communication", num = 6, name = "Communication";
    const t0 = Date.now();
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    const id = "votes", num = 7, name = "Votes";
    const t0 = Date.now();
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 8: Citation (plenary agenda)
  // ═══════════════════════════════════════════════════════════════════════════
  {
    const id = "citation", num = 8, name = "Citation";
    const t0 = Date.now();
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 9: Official Gazette (OJ TOC)
  // ═══════════════════════════════════════════════════════════════════════════
  {
    const id = "gazette", num = 9, name = "Gazette";
    const t0 = Date.now();
