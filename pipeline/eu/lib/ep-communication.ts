/**
 * EP Open Data → Akoma Ntoso communication document
 *
 * Downloads legislative procedure lifecycle data from the European Parliament
 ...
 * a procedure event where one institution sends a document to another.
 */
...
/**
 * Fetch EP procedure lifecycle data and build an AKN communication document.
 *
 * @param procYear   - Procedure year (e.g. "2024"), used only for the label
 * @param procApiId  - Direct EP API procedure ID (e.g. "115672")
 * @param procType   - Procedure type code (e.g. "COD")
 * @param outputPath - Full path where the XML file will be written
 */
export async function buildCommunication(
	procYear: string,
	procApiId: string,
	procType: string,
	outputPath: string
): Promise<void> {
	const procId = procApiId;
	const procLabel = `${procApiId}(${procType})`;
	console.log(`  Selected: ${procLabel} (${procId})`);

	// Fetch full procedure detail
	const procUrl = `${EP_API}/api/v2/procedures/${procId}?format=application%2Fld%2Bjson`;
	...
	if (!procDetail) {
		throw new Error('No procedure detail returned');
	}

	const titleEn: string = procDetail.process_title?.en ?? `Procedure ${procLabel}`;
	...
	// Procedure URI
	const procUri = `${EP_API}/${procDetail.id}`;
	...
	x.selfClose('TLCProcess', {
		eId: 'cod',
		href: `${EP_API}/def/ep-procedure-types/COD`,
	...
	x.inline('p', {}, `Current stage: ${esc(stageName(currentStage))}`);
	x.close('longTitle');
	x.close('preface');
	...
	for (let i = 0; i < events.length; i++) {
		...
		const relDocs: string[] =
			ev.based_on_a_realization_of ?? ev.decided_on_a_realization_of ?? [];
		if (relDocs.length > 0) {
			for (const docRef of relDocs) {
				...
				x.inline(
					'p',
					{ class: 'document' },
					`<ref href="${esc(`${EP_API}/${docRef}`)}">${esc(docLabel)}</ref>`
				);
			}
		}
		...
		if (recordedIn.length > 0) {
			for (const recRef of recordedIn) {
				...
				x.inline(
					'p',
					{ class: 'record' },
					`Recorded in: <ref href="${esc(`${EP_API}/${recRef}`)}">${esc(recLabel)}</ref>`
				);
			}
		}
	...
	if (participations.length > 0) {
		...
		for (const p of participations) {
			...
			if (persons.length > 0) {
				...
				line += `<person refersTo="${esc(`${EP_API}/${persons[0]}`)}">${esc(personId)}</person>`;
				if (group) line += ` (${esc(group)})`;
				if (committee) line += ` for ${esc(committee)}`;
				if (date)
					line += ` — <date date="${esc(date)}">${esc(formatDate(date))}</date>`;
				x.inline('p', { class: 'participant' }, line);
			} else if (orgs.length > 0) {
				const orgId = orgs[0].split('/').pop() ?? orgs[0];
				let line = `<role refersTo="#${esc(role.toLowerCase())}">${esc(roleName)}</role>: `;
				line += `<organization refersTo="${esc(`${EP_API}/${orgs[0]}`)}">${esc(orgId)}</organization>`;
				x.inline('p', { class: 'participant' }, line);
			}
		}
