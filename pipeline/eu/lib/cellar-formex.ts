...
// ─── Shared core: fetch metadata and iterate DOC URLs ────────────────────────

async function fetchDocUrls(celex: string): Promise<{ docUrls: string[]; usedLang: string }> {
	...
	const encodedCelex = celex.replace(/\(/g, "%28").replace(/\)/g, "%29");
	const noticeUrl = `${CELLAR_BASE}/${encodedCelex}`;

	// Try EN first, then fallback languages
	const LANGS = ["en", "fra", "deu", "mul"];
	let notice: { status: number; buffer: Buffer; contentType: string } | null = null;
	let usedLang = "en";
	for (const lang of LANGS) {
		const attempt = await fetchBuffer(noticeUrl, "application/xml;notice=branch", lang);
		if (attempt.status === 200) {
			notice = attempt;
			usedLang = lang;
			if (lang !== "en") console.log(`   No English version — using ${lang}`);
			break;
		}
		if (attempt.status !== 406) {
			// Not a language issue — real error
			notice = attempt;
			break;
		}
	}

	if (!notice || notice.status !== 200) {
		console.error(`   HTTP ${notice?.status ?? "?"} — CELEX not available in any language.`);
		console.error(`   ${notice?.buffer.toString("utf-8").slice(0, 200) ?? ""}`);
		throw new Error(`CELEX ${celex} not available (HTTP ${notice?.status ?? "?"})`);
	}
	...
	if (docUrls.length === 0) {
		throw new Error(`No DOC URLs found for CELEX ${celex}. This CELEX may not have downloadable Formex.`);
	}

	return { docUrls, usedLang };
}

// ─── Save helpers ────────────────────────────────────────────────────────────

function saveTocResult(xml: string, celex: string, outputDir: string) {
	mkdirSync(outputDir, { recursive: true });
	const outPath = `${outputDir}/${celex}-toc-formex.xml`;
...
function saveResult(xml: string, celex: string, outputDir: string) {
	mkdirSync(outputDir, { recursive: true });
	const outPath = `${outputDir}/${celex}-formex.xml`;
	writeFileSync(outPath, xml, "utf-8");
...
/**
 * Downloads the Formex body XML for the given CELEX number.
 * Saves the result as {celex}-formex.xml in the output directory.
 */
export async function downloadFormex(celex: string, outputDir: string): Promise<void> {
	const { docUrls } = await fetchDocUrls(celex);
	...
	for (const docUrl of docUrls) {
		...
		if (ct.includes("fmx4") || (ct.includes("xml") && !ct.includes("xhtml") && !ct.includes("rdf"))) {
			...
			if (isFormexBody(text)) {
				...
				return saveResult(text, celex, outputDir);
			}
		...
		if (ct.includes("zip")) {
			...
			// Collect all XML entries with Formex body (skip TOC)
			const candidates: { name: string; text: string; isCorr: boolean }[] = [];
			for (const entry of entries) {
				if (entry.name.includes("toc")) continue;
				if (!entry.name.endsWith(".xml") && !entry.name.endsWith(".fmx.xml")) continue;
				const text = entry.data.toString("utf-8");
				if (isFormexBody(text)) {
					candidates.push({ name: entry.name, text, isCorr: text.includes("<CORR") && !text.includes("<DOC.CORR") });
				}
			}
			// Prefer <CORR> body over <DOC.CORR> wrapper
			const best = candidates.find(c => c.isCorr) || candidates[0];
			if (best) {
				console.log(`     ✓ ${best.name} has Formex body (${best.text.length} bytes)`);
				return saveResult(best.text, celex, outputDir);
			}
	...
	throw new Error(`No Formex body found for CELEX ${celex}. Possible reasons:\n  - This CELEX is a COM proposal (5xxxx) → use the bill downloader instead\n  - This document only has PDF/XHTML, no Formex XML\n  - The Formex may be under a different expression/language`);
}

/**
 * Downloads the Formex TOC XML for the given CELEX number.
 * Saves the result as {celex}-toc-formex.xml in the output directory.
 */
export async function downloadFormexToc(celex: string, outputDir: string): Promise<void> {
	const { docUrls } = await fetchDocUrls(celex);

	// Step 2: Try each DOC URL
	console.log("\n2. Checking each DOC for Formex TOC content...");
	...
	for (const docUrl of docUrls) {
		...
		// Case A: Direct XML — no TOC in raw XML mode
		if (ct.includes("fmx4") || (ct.includes("xml") && !ct.includes("xhtml") && !ct.includes("rdf"))) {
			const text = resp.buffer.toString("utf-8");
			console.log(`XML (${text.length} bytes, skip — TOC only in ZIP)`);
			continue;
		}

		// Case B: ZIP containing .fmx.xml files — look for TOC
		if (ct.includes("zip")) {
			console.log(`ZIP (${resp.buffer.length} bytes) — extracting...`);
			const entries = extractZipEntries(resp.buffer);
			console.log(`     ${entries.length} files in ZIP: ${entries.map(e => e.name).join(", ")}`);
			...
			for (const entry of entries) {
				if (entry.name.includes("toc") && (entry.name.endsWith(".fmx.xml") || entry.name.endsWith(".xml"))) {
					const text = entry.data.toString("utf-8");
					console.log(`     ✓ TOC: ${entry.name} (${text.length} bytes)`);
					return saveTocResult(text, celex, outputDir);
				}
			}
			console.log("     No TOC file found in ZIP entries");
			continue;
		}
	...
	throw new Error(`No Formex TOC found for CELEX ${celex}. This document may not have a TOC in Formex format.`);
}
