/**
 * CELLAR XHTML → Akoma Ntoso (AKN 3.0) Bill Converter
 *
 ...
 * available — only XHTML and PDF. This module uses content negotiation
...
export async function downloadBill(celex: string, outputPath: string): Promise<void> {
	const url = `${CELLAR_BASE}/${celex}`;

	// Step 1: Download XHTML from CELLAR
	const xhtml = await fetchXhtml(url);

	// Save raw XHTML for reference
	const rawPath = join(dirname(outputPath), `${celex}-raw.xhtml`);
