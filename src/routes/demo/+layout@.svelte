<script lang="ts">
	...
	function tabHref(g: TabGroupId): string {
		...
		if (g === 'graph') return `/demo/${country}/graph`;
	...
	function switchCountry(nextCode: string) {
		...
		if (activeGroup === 'graph') {
			goto(`/demo/${nextCode}/graph`);
			return;
		}
				...
				<div class="flex items-center gap-2">
					...
					<span class="org-trigger relative inline-block h-5 w-5">
						...
						>
							...
							{/if}
							<option value="__home">← All organizations</option>
		...
		{#if country}
			<nav class="bg-white">
				<div class="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-4">
					{#each TAB_GROUPS as g, i (g.id)}
						{@const Icon = tabIcons[g.id]}
						{@const active = activeGroup === g.id}
						<a
							href={tabHref(g.id)}
							class="-mb-px flex items-center gap-1.5 border-b-2 px-3 py-2 text-sm font-medium whitespace-nowrap transition-colors {i === 0
								? '-ml-3'
								: ''} {active
